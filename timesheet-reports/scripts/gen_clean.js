// gen_clean.js — clean dashboard generator with correct emoji (UTF-8)
const fs = require('fs');
const path = require('path');

const BASE = path.resolve(__dirname, '..');
const cfg = JSON.parse(fs.readFileSync(path.join(BASE, 'config.json'), 'utf8'));
const COMPANY = (cfg.company && cfg.company.name) || 'PT TwinK Digital Indonesia';
...
const HOL_ICONS = [
  ['Natal','🎄'],['Imlek','🧧'],['Isra','🌙'],['Mi\'raj','🌙'],['Nyepi','🪔'],['Idulfitri','🌙'],['Idul Fitri','🌙'],
  ['Iduladha','🐑'],['Idul Adha','🐑'],['Paskah','⛪'],['Kebangkitan','⛪'],['Jumat Agung','✝️'],['Agung','✝️'],
  ['Kenaikan','✝️'],['Waisak','☸️'],['Pancasila','🇮🇩'],['Kemerdekaan','🇮🇩'],['Maulid','🌙'],['Buruh','⚙️'],
  ['Tahun Baru','🎊'],['Cuti Bersama','🏖️']
];
const DNAMES = ['Senin','Selasa','Rabu','Kamis','Jumat','Sabtu','Minggu'];
const RCLS = ['r1','r2','r3','r3', ...Array(40).fill('rn')];
const RMED = ['🥇','🥈','🥉','🥉', ...Array(40).fill('')];

function esc(s){ return String(s ?? '').replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;'); }
function daysInMonth(y,m){ return new Date(Date.UTC(y,m,0)).getUTCDate(); }
function dow(y,m,d){ return (new Date(Date.UTC(y,m-1,d)).getUTCDay()+6)%7; } // Mon=0
...
const staffArr = Object.entries(data.staff||{}).map(([name,s])=>({
  name, h:Number(s.total_hours||0), daily:s.daily||{}, projects:s.projects||{},
  days: Object.entries(s.daily||{}).filter(([d,h])=> !sats.has(Number(d)) && !suns.has(Number(d)) && Number(h)>0).length
})).sort((a,b)=> b.h-a.h || a.name.localeCompare(b.name));

staffArr.forEach((s,i)=>{
  s.pct = wd ? Math.round(s.days/wd*1000)/10 : 0;
  s.color = COLORS[i%COLORS.length];
  const top = Object.entries(s.projects).sort((a,b)=>b[1]-a[1])[0];
  s.top = top ? String(top[0]).slice(0,45) : '–';
});

const totalH = staffArr.reduce((a,s)=>a+s.h,0);
const aboveTgt = staffArr.filter(s=>s.h>=tgt).length;
const maxH = Math.max(...staffArr.map(s=>s.h), 1);

const projMap={};
for(const s of Object.values(data.staff||{})) for(const [k,v] of Object.entries(s.projects||{})) if(Number(v)>0) projMap[k]=(projMap[k]||0)+Number(v);
const projTotals=Object.entries(projMap).sort((a,b)=>b[1]-a[1]);
const maxP=projTotals.length?projTotals[0][1]:1;

// holiday cards
let hol_html='';
const wkd_h = Object.entries(hols).filter(([d,n])=> dow(year,mo,Number(d))<5);
for(const [ds,hn] of Object.entries(hols).sort((a,b)=>Number(a[0])-Number(b[0]))){
  const w=dow(year,mo,Number(ds)); const hari=DNAMES[w];
  const ic=holIcon(hn); const isSun=w===6, isSat=w===5, isWknd=w>=5;
  ...
  const bg=isWknd?'#fef9c3':'#fee2e2', brd=isWknd?'#fde68a':'#fca5a5', fc=isWknd?'#854d0e':'#b91c1c', fc2=isWknd?'#713f12':'#991b1b', fc3=isWknd?'#78350f':'#7f1d1d';
  hol_html+=`<div class="hol-card" style="background:${bg};border:1.5px solid ${brd}"><div class="hico">${ic}</div><div><div class="hdate" style="color:${fc}">${hari}, ${ds} ${mid} ${year}</div><div class="hname" style="color:${fc2}">${esc(hn)}</div><div class="hnote" style="color:${fc3}">${note}</div></div></div>`;
}
hol_html+=`<div class="hol-card" style="background:#dbeafe;border:1.5px solid #93c5fd"><div class="hico">🧮</div><div><div class="hdate" style="color:#1e40af">Perhitungan Hari Kerja</div><div class="hname" style="color:#1e3a8a">${numDays} − ${sats.size} Sabtu − ${suns.size} Minggu − ${wkd_h.length} Libur = <strong>${wd} hari</strong></div><div class="hnote" style="color:#1d4ed8">Target: ${wd} × 8 = <strong>${tgt} jam / orang</strong></div></div></div>`;

// calendar
const dm={}; for(let d=1;d<=numDays;d++) dm[d]={tot:0,staff:[]};
for(const s of staffArr) for(const [ds2,h] of Object.entries(s.daily)){ const d=Number(ds2); if(Number(h)>0){ dm[d].tot+=Number(h); dm[d].staff.push({c:s.color,n:s.name,h:Number(h)}); } }
const fdow=dow(year,mo,1);
let cal='';
for(let i=0;i<fdow;i++) cal+='<div class="cell empty"></div>';
for(let d=1;d<=numDays;d++){
  const isSat=sats.has(d), isSun=suns.has(d), isHol=String(d) in hols, hasData=dm[d].tot>0;
  ...
  cal+=`<div class="${cls}"><div class="cn">${d}</div>${ty?`<div class=ct>${ty}</div>`:''}${htag}${dots}${ctot}</div>`;
}

// scorecards
let sc='';
staffArr.forEach((s,i)=>{
  const w=Math.round(s.h/maxH*1000)/10;
  sc+=`<div class="sc ${RCLS[i]}"><div class="rnum">${i+1}</div>${RMED[i]?`<span class=sc-crn>${RMED[i]}</span>`:''}<div class="sc-name">${esc(s.name)}</div><div class="sc-val" style="color:${s.color}">${s.h.toFixed(0)}<span style="font-size:1rem;font-weight:600"> jam</span></div><div class="sc-sub">${s.days} hari diisi &nbsp;·&nbsp; ${s.pct}% pengisian</div><div class="prog"><div class="pbar" style="width:${w}%;background:${s.color}"></div></div><div class="sc-meta"><span>📁 ${esc(s.top)}</span></div></div>`;
});

// bar charts
let bh='', bp='';
for(const s of staffArr){
  const sh=s.name.split(' ')[0]; const wh=Math.round(s.h/maxH*1000)/10; const wp=Math.round(Math.min(s.pct/110*100,100)*10)/10;
  bh+=`<div class="bl-row"><div class="bl-lbl">${esc(sh)}</div><div class="bl-track"><div class="bl-fill" style="width:${wh}%;background:${s.color}">${s.h.toFixed(0)}j</div></div><div class="bl-val" style="color:${s.color}">${s.h.toFixed(0)}j</div></div>`;
  bp+=`<div class="bl-row"><div class="bl-lbl">${esc(sh)}</div><div class="bl-track"><div class="bl-fill" style="width:${wp}%;background:${s.color}">${s.pct>=100?'✓ ':''}${s.pct}%</div></div><div class="bl-val" style="color:${s.color}">${s.pct}%</div></div>`;
}

// projects
let pj='';
projTotals.forEach(([pn,ph],i)=>{
  const w=Math.round(ph/maxP*1000)/10; const c=PCOLS[i%PCOLS.length];
...
staffArr.forEach((s,i)=>{
  const diff=s.h-tgt;
  const ds2=diff>=0?`<span style="color:#22c55e;font-weight:700">+${diff.toFixed(0)}j</span>`:`<span style="color:#ef4444;font-weight:700">${diff.toFixed(0)}j</span>`;
  let st,pc;
  if(s.h>=tgt*1.1){st='Excellent';pc='p-gr';} else if(s.h>=tgt){st='Lengkap';pc='p-bl';} else if(s.h>=tgt*0.9){st='Hampir';pc='p-yw';} else {st='Kurang';pc='p-rd';}
  tb+=`<tr><td>${RMED[i]}${i+1}</td><td><strong>${esc(s.name)}</strong></td><td>${s.h.toFixed(0)} jam</td><td>${s.days} hari</td><td><span class="pill p-bl">${s.pct}%</span></td><td>${ds2}</td><td><span class="pill ${pc}">${st}</span></td><td style="font-size:.71rem;color:#94a3b8">${esc(s.top)}</td></tr>`;
});

const CSS = `:root{--bg:#f0f4ff;--card:#fff;--border:#e2e8f0;--text:#334155;--muted:#94a3b8;--r:14px;--sh:0 2px 16px rgba(99,102,241,.08);--sat-bg:#fef9c3;--sat-b:#fde68a;--sat-fg:#854d0e;--sun-bg:#ffedd5;--sun-b:#fdba74;--sun-fg:#9a3412;--hol-bg:#fee2e2;--hol-b:#fca5a5;--hol-fg:#b91c1c;--work:#f8faff;--data-bg:#eff6ff;--data-b:#bfdbfe;--data-fg:#1e40af}
...
<div class="hdr">
  ...
  <p>${COMPANY} &nbsp;·&nbsp; Summary Report Bulanan</p></div>
  <div class="badges">
...
<div class="stat-row">
  ...
  <div class="stat" style="border-left-color:#0ea5e9"><div class="ico">✅</div><div class="sv">${aboveTgt}/${staffArr.length}</div><div class="sl">Staff ≥ Target Jam</div></div>
  <div class="stat" style="border-left-color:#8b5cf6"><div class="ico">🏆</div><div class="sv">${staffArr.length?esc(staffArr[0].name.split(' ')[0]):'-'}</div><div class="sl">Jam Terbanyak</div></div>
</div>
...
<p class="sec">🏆 Leaderboard — Total Jam Diisi (${mid} ${year})</p>
...
<p class="sec">Beban Jam per Project — ${mid} ${year}</p>
...
<div class="footer">Timesheet Dashboard ${mid} ${year} &nbsp;&middot;&nbsp; ${COMPANY}</div>
</body></html>
`;

if(!outArg){
  const outDir = path.join(BASE,'output', mid.toLowerCase()+year);
  fs.mkdirSync(outDir,{recursive:true});
  outArg = path.join(outDir, `Dashboard Timesheet ${mid} ${year}.html`);
}
outArg = path.resolve(outArg);
fs.mkdirSync(path.dirname(outArg),{recursive:true});
fs.writeFileSync(outArg, html, 'utf8');
console.log('Dashboard saved -> ' + outArg);
console.log('Staff count: ' + staffArr.length + ', total hours: ' + totalH.toFixed(0));
